...
#define KEYVALUE(unshifted, shifted)	( ((uint8_t)unshifted) | (((uint8_t)shifted) << 8) )
#define UNSHIFTED(value)	((char) value)
#define SHIFTED(value)	((char) (value >> 8) )

#define OPEN LOW
#define CLOSED HIGH

typedef uint16_t KeypadEvent;

#define LIST_MAX 10		// Max number of keys on the active list.
#define MAPSIZE 10		// MAPSIZE is the number of rows (times 16 columns)
#define makeKeymap(x) ((uint16_t *)x)

class Keypad {
public:

	Keypad(const uint16_t *userKeymap);
	...
	void addEventListener(void (*listener)(uint16_t));
	int16_t findInListByCharacter(uint16_t keyChar);
	int16_t findInListByCode(int16_t keyCode);
	uint16_t waitForKey();
...
private:
	...
	void (*keypadEventListener)(uint16_t);
};
