...
//------------------------------------------------------------------------------
/**
 * Read a line of G Code text from a file starting at the current position and null-terminates the buffer read.
 *
 * \param[out] buf Pointer to the location that will receive the data (must hold nbyte + 1 bytes).
 *
 * \param[in] nbyte Maximum number of bytes to read.
 *
 * \return For success readGCodeLine() returns the number of bytes read.
 * A value less than \a nbyte, including zero, will be returned
 * if end of file is reached.
 * If an error occurs, readGCodeLine() returns -1.  Possible errors include
 * a line too long to fit in the buffer passed in, readGCodeLine() called 
 * before a file has been opened, corrupt file system or an I/O error
 * occurred.
 */
int16_t SdFile::readGCodeLine(void* buf, uint16_t nbyte) {
  uint8_t* dst = reinterpret_cast<uint8_t*>(buf);

  // error if not open or write only
  if (!isOpen() || !(flags_ & O_READ)) return -1;

  uint16_t toRead; // amount left to read
  if(nbyte > (fileSize_ - curPosition_))
  {
    toRead = fileSize_ - curPosition_;
  }
  else
  {
    toRead = nbyte;
  ...
  while (toRead > 0) {
    ...
    uint16_t n = toRead; // amount to be read from current block
    if (n > (512 - offset)) n = 512 - offset;
    ...
    while (src != end) {
      ...
      switch (c) {
        ...
        default:
          ...
          if (c >= 'a' && c <= 'z') c = c-'a'+'A'; // Upcase lowercase
    ...
    *dst = 0;
    if ( !didReadOK && ( fileSize_ - curPosition_  > 0 ) ) return -1; // The line was too long.
    else return dst - (reinterpret_cast<uint8_t*>(buf));
}
//------------------------------------------------------------------------------
/**
